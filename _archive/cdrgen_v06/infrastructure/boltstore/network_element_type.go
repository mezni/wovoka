...
// CreateMany inserts a slice of NetworkElementTypes into the database.
func (r *NetworkElementTypeRepository) CreateMany(elements []entities.NetworkElementType) ([]entities.NetworkElementType, error) {
...
// GetMaxID retrieves the maximum ID assigned to NetworkElementTypes in the database.
func (r *NetworkElementTypeRepository) GetMaxID() (int, error) {
	var maxID int
	err := r.db.View(func(tx *bolt.Tx) error {
		b := tx.Bucket([]byte(r.bucketName))
		c := b.Cursor()
